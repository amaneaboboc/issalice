...
//        range_y = max_y - min_y;
        ...
        h_n1_pt  = createHistogram(createName(bn,"n1_pt"), nBins_pt,  min_pt,  max_pt,  "p_{T}","N");
	      ...
	      h_events = createHistogram(createName(bn,"events"),1,0,1, "No. Events","bin");	
	
        if(fillEta)
        {
            h_n1_phiEta  = createHistogram(createName(bn,"n1_phiEta"), nBins_eta, min_eta, max_eta, nBins_phi, min_phi, max_phi, "#eta", "#varphi","N");
            h_pt_phiEta  = createHistogram(createName(bn,"pt_phiEta"), nBins_eta, min_eta, max_eta, nBins_phi, min_phi, max_phi, "#eta", "#varphi","p_T");
    ...
    void fill(TrackN* trk, double weight)
    {
        double pt = trk->pt;
        int charge = trk->q;
        ...
        h_pt_phiEta->Fill(trkEta,phiNew,pt);
        ...
        void fillEvents(double nAccepted)
        {
...
void fill(TrackN* trk1, TrackN* trk2, double weight)
{
  ...
  double pt1 = trk1->pt; iPt1 = fillP2 ? getPtBinFor(pt1) : 0;

  double phi1 = trk1->phi;    if(phi1<0.0){ phi1+= TMath::TwoPi();} iPhi1 = getPhiBinFor(phi1);

  double eta1 = trk1->eta;    iEta1 = fillEta ? getEtaBinFor(eta1) : 0;

  //double y1 = 0.5 * log((trk1->e + trk1->pz)/(trk1->e - trk1->pz));
        //iY1 = fillY ? getYBinFor(y1) : 0;
  
  double pt2 = trk2->pt; iPt2 = fillP2 ? getPtBinFor(pt2) : 0;

  double phi2 = trk2->phi;    if(phi2<0.0){ phi2+= TMath::TwoPi();} iPhi2 = getPhiBinFor(phi2);
  
  double eta2 = trk2->eta;    iEta2 = fillEta ? getEtaBinFor(eta2) : 0;
  
  //double y2 = 0.5 * log((trk2->e + trk2->pz)/(trk2->e - trk2->pz));
        //iY2 = fillY ? getEtaBinFor(y2) : 0;
  
  //iEta2 = getEtaBinFor(trk2->eta);//static_cast<int>(( (trk2->eta) - min_eta)/etabinwidth);
  //if(trk2->phi <= 0){phi2=trk2->phi;phi2 += 6.2831853;}  
  //iPhi2 = static_cast<int>((trk2->phi+TMath::Pi()) * double(nBins_phi)/range_phi);
  ...
  h_DptDpt_DetaDphi->AddBinContent(iGDeltaEtaDeltaPhi, weight*pt1*pt2);
  h_DptDpt_DetaDphi->SetEntries(h_DptDpt_DetaDphi->GetEntries()+1);
        ...
        h_n2_DetaDphi->Write();
        //h_n2_DyDphi->Write();
        h_DptDpt_DetaDphi->Write();
        }
        
        void Scale(Int_t nEv)
        {
        Double_t norm;
	      //<<"nr of events "<<nEv<<endl;
	      norm = 1/Double_t(nEv);
	      h_n2_DetaDphi->Scale(norm);
        h_DptDpt_DetaDphi->Scale(norm);
...
public:
  ...
  TH2D* h_pt1pt1_DetaDphi;
  TH2D* h_G2_DetaDphi;
  TH2D* h_G2_DetaDphi_shft;
...
if(fillY){
    ...
    h_n1n1_DyDphi_shft  = createHistogram(createName(bn,"n1n1_DyDphi_shft"),  nBins_Dy,  min_Dy,  max_Dy,  nBins_Dphi,  min_Dphi_shft, max_Dphi_shft, "#Delta y", "#Delta#varphi", "<n_{1}><n_{1}>");
...
if(fillP2){
      ...
      //double avgPt1Eta = avgValue(part1BaseHistos->h_n1_pt);
      //double avgPt2Eta = avgValue(part2BaseHistos->h_n1_pt);
      //double scalingFactorEta = 1.0/avgPt1Eta/avgPt2Eta;

      //h_P2_DetaDphi->Scale(scalingFactorEta);

      reduce_n1EtaPhiN1EtaPhiOntoN1N1DetaDphi(part1BaseHistos->h_pt_phiEta,part2BaseHistos->h_pt_phiEta,h_pt1pt1_DetaDphi,nBins_Deta,nBins_Dphi);

      calculateG2_H2H2H2H2(pairHistos->h_DptDpt_DetaDphi, h_n1n1_DetaDphi, h_pt1pt1_DetaDphi, h_G2_DetaDphi, 0, bincorrection, 1.0);
	...
	reduce_n1EtaPhiN1EtaPhiOntoN1N1DetaDphi(part1BaseHistos->h_n1_phiY,part2BaseHistos->h_n1_phiY,h_n1n1_DyDphi,nBins_Dy,nBins_Dphi);
	    ...
	    shiftY(*h_n1n1_DyDphi,*h_n1n1_DyDphi_shft,   nBins_Dphi_shft);
...
const std::vector<ParticleSingleHistos*> &get_sgHist() const { return HistogramSet; }

const std::vector<ParticlePairHistos*> &get_pairHist() const { return HistogramPairSet; }
