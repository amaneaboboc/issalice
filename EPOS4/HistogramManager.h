...
public:
  ...
  ParticleSingleHistos (const TString &_name)
      : // HistogramGroup(_name),
        fillEta (1), fillY (0), nBins_n1 (100), min_n1 (0), max_n1 (1000),
        nBins_pt (100), min_pt (0.2), max_pt (2.0), nBins_phi (72),
        min_phi (0), max_phi (TMath::TwoPi ()), nBins_eta (20), min_eta (-1.0),
        max_eta (1.0), range_eta (0), nBins_y (0), min_y (0), max_y (0),
        range_y (0), h_n1 (nullptr),
        // h_n1_eTotal(nullptr),
        h_n1_pt (nullptr),
        // h_n1_ptXS(nullptr),
        h_n1_phiEta (nullptr), h_n1_phiY (nullptr),
  ...
  createHistograms ()
  {
    ...
    h_events = createHistogram (createName (bn, "events"), 1, 0, 1,
                                "No. Events", "bin");
    ...
    if (fillY)
      {
        h_n1_phiY = createHistogram (createName (bn, "n1_phiY"), nBins_y,
                                     min_y, max_y, nBins_phi, min_phi, max_phi,
                                     "y", "#varphi", "N");
      }
  }

  void
  fill (Track trk, double weight)
  {
  ...
  void
  fillMultiplicity (double nAccepted, double weight)
  {
    h_n1->Fill (nAccepted, weight);
  }

  void
  fillEvents (double nAccepted)
  {
    h_events->Fill (nAccepted);
  }
...
public:
  ...
  ParticlePairHistos (const TString &_name)
      : // HistogramGroup(_name),
        fillEta (1), fillY (0), nBins_n2 (100), min_n2 (0), max_n2 (1000),
        nBins_pt (100), min_pt (0.2), max_pt (2.0), nBins_phi (72),
        min_phi (0), max_phi (TMath::TwoPi ()), range_phi (0), scale_phi (0),
        nBins_eta (20), min_eta (-1.0), max_eta (1.0), range_eta (0),
        scale_eta (0), etabinwidth (0), nBins_y (0), min_y (0), max_y (0),
        range_y (0), scale_y (0), nBins_Deta (0), min_Deta (0), max_Deta (0),
        range_Deta (0), nBins_Dy (0), min_Dy (0), max_Dy (0), nBins_Dphi (0),
        min_Dphi (0), max_Dphi (0), range_Dphi (0), width_Dphi (0),
        min_Dphi_shft (0), max_Dphi_shft (0), h_n2 (nullptr),
  ...
  */
  inline int
  getPhiBinFor (float v) const
  {
    int index = 0; // indicates a value out of bounds
    if (v < min_phi || v >= max_phi)
      return index;
  ...
  inline int
  getEtaBinFor (float v) const
  {
    int index = 0; // indicates a value out of bounds
    if (v < min_eta || v >= max_eta)
      return index;
  ...
  void
  fill (TrackN *trk1, TrackN *trk2, double weight)
  {
...
public:
  ...
  PairDerivedHistos (const TString &_name)
      ...
      max_Dphi_shft(0),
      range_y(0),*/
        h_n1n1_DetaDphi (nullptr), h_rho2_DetaDphi (nullptr),
        h_A2_DetaDphi (nullptr), h_B2_DetaDphi (nullptr),
        h_C2_DetaDphi (nullptr), h_R2_DetaDphi (nullptr)
  {
    // appendClassName("ParticleSingleHistos");
    name = _name;
  }
  ...
  createHistograms ()
  {
    ...
    if (fillEta)
      {
        h_n1n1_DetaDphi = createHistogram (
            createName (bn, "n1n1_DetaDphi"), nBins_Deta, min_Deta, max_Deta,
            nBins_Dphi, min_Dphi, max_Dphi, "#Delta#eta", "#Delta#varphi",
            "<n_{1}><n_{1}>");
        h_rho2_DetaDphi = createHistogram (
            createName (bn, "rho2_DetaDphi"), nBins_Deta, min_Deta, max_Deta,
            nBins_Dphi, min_Dphi, max_Dphi, "#Delta#eta", "#Delta#varphi",
            "#rho_{2}");
        h_A2_DetaDphi = createHistogram (
            createName (bn, "A2_DetaDphi"), nBins_Deta, min_Deta, max_Deta,
            nBins_Dphi, min_Dphi, max_Dphi, "#Delta#eta", "#Delta#varphi",
            "A_{2}");
        h_B2_DetaDphi = createHistogram (
            createName (bn, "B2_DetaDphi"), nBins_Deta, min_Deta, max_Deta,
            nBins_Dphi, min_Dphi, max_Dphi, "#Delta#eta", "#Delta#varphi",
            "B_{2}");
        h_C2_DetaDphi = createHistogram (
            createName (bn, "C2_DetaDphi"), nBins_Deta, min_Deta, max_Deta,
            nBins_Dphi, min_Dphi, max_Dphi, "#Delta#eta", "#Delta#varphi",
            "C_{2}");
        ...
        h_R2_DetaDphi = createHistogram (
            createName (bn, "R2_DetaDphi"), nBins_Deta, min_Deta, max_Deta,
            nBins_Dphi, min_Dphi, max_Dphi, "#Delta#eta", "#Delta#varphi",
            "R_{2}");

        h_rho2_DetaDphi_shft = createHistogram (
            createName (bn, "rho2_DetaDphi_shft"), nBins_Deta, min_Deta,
            max_Deta, nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta#eta",
            "#Delta#varphi", "#rho_{2}");
        h_A2_DetaDphi_shft = createHistogram (
            createName (bn, "A2_DetaDphi_shft"), nBins_Deta, min_Deta,
            max_Deta, nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta#eta",
            "#Delta#varphi", "A_{2}");
        h_B2_DetaDphi_shft = createHistogram (
            createName (bn, "B2_DetaDphi_shft"), nBins_Deta, min_Deta,
            max_Deta, nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta#eta",
            "#Delta#varphi", "B_{2}");
        h_C2_DetaDphi_shft = createHistogram (
            createName (bn, "C2_DetaDphi_shft"), nBins_Deta, min_Deta,
            max_Deta, nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta#eta",
            "#Delta#varphi", "C_{2}");
        // h_D2_DetaDphi_shft   =
        ...
        h_R2_DetaDphi_shft = createHistogram (
            createName (bn, "R2_DetaDphi_shft"), nBins_Deta, min_Deta,
            max_Deta, nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta#eta",
            "#Delta#varphi", "R_{2}");
      }
    ...
    if (fillY)
      {
        h_n1n1_DyDphi
            = createHistogram (createName (bn, "n1n1_DyDphi"), nBins_Dy,
                               min_Dy, max_Dy, nBins_Dphi, min_Dphi, max_Dphi,
                               "#Delta y", "#Delta#varphi", "<n_{1}><n_{1}>");
        h_rho2_DyDphi
            = createHistogram (createName (bn, "rho2_DyDphi"), nBins_Dy,
                               min_Dy, max_Dy, nBins_Dphi, min_Dphi, max_Dphi,
        ...
        h_rho2_DyDphi_shft = createHistogram (
            createName (bn, "rho2_DyDphi_shft"), nBins_Dy, min_Dy, max_Dy,
            nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta y",
            "#Delta#varphi", "#rho_{2}>");
        h_A2_DyDphi_shft = createHistogram (
            createName (bn, "A2_DyDphi_shft"), nBins_Dy, min_Dy, max_Dy,
            nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta y",
            "#Delta#varphi", "A_{2}");
        h_B2_DyDphi_shft = createHistogram (
            createName (bn, "B2_DyDphi_shft"), nBins_Dy, min_Dy, max_Dy,
            nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta y",
            "#Delta#varphi", "B_{2}");
        h_C2_DyDphi_shft = createHistogram (
            createName (bn, "C2_DyDphi_shft"), nBins_Dy, min_Dy, max_Dy,
            nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta y",
            "#Delta#varphi", "C_{2}");
        h_R2_DyDphi_shft = createHistogram (
            createName (bn, "R2_DyDphi_shft"), nBins_Dy, min_Dy, max_Dy,
            nBins_Dphi, min_Dphi_shft, max_Dphi_shft, "#Delta y",
            "#Delta#varphi", "R_{2}");
      }
  }
  ...
  void
  calculatePairDerivedHistograms (ParticleSingleHistos *part1BaseHistos,
    ...
    if (fillY)
      {
        ...
        reduce_n1EtaPhiN1EtaPhiOntoN1N1DetaDphi (
            part1BaseHistos->h_n1_phiY, part2BaseHistos->h_n1_phiY,
            h_n1n1_DyDphi, nBins_Dy, nBins_Dphi);
...
public:
  ...
  void
  addHistoInSet (unsigned int index, ParticleSingleHistos *group)
  {
  ...
  void
  addHistoInSet (unsigned int index, ParticlePairHistos *group)
  {
  ...
  void
  addHistoInSet (unsigned int index, PairDerivedHistos *group)
  {
  ...
  void
  getHistoInSet (unsigned int index, ParticleSingleHistos *hist)
  {
  ...
  const std::vector<ParticleSingleHistos *> &
  get_sgHist () const
  {
    return HistogramSet;
  }

  const std::vector<ParticlePairHistos *> &
  get_pairHist () const
  {
